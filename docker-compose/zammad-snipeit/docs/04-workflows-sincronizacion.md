...
- `Create/Update snipe-IT asset (batch)`: `batchSize:1, batchInterval:550` + `retryOnFail (4×3s)` + `onError: continueRegularOutput` + `fullResponse` — evita `429` (throttle 60/min) y reintenta. `Find asset in Snipe` también con retry 3×2s.
...
**Archivo:** `flows/flujos-dev/Tryton sync users assets.json` (ID `yjyUYjVEaZ9UniSs`, activo) — invocado por el orquestador vía `Sync titular activo` (`waitForSubWorkflow: true`). También corre manual.
...
Iniciar sesión en Tryton (Cnq2yvzVCRTKFld5) → Leer activos con titular (retry 3×3s) → Extraer activos y titulares (dedup owner_ids)
     ...
     → Precargar usuarios de Snipe-IT (GET /users?limit=500, 1 req) → Guardar mapa de usuarios (bulk) (jsonb_to_recordset ON CONFLICT DO UPDATE)
      → Calcular usuarios faltantes (DISTINCT ON LOWER(email) no en mapa) → ¿Hay usuarios faltantes? → Extraer usuarios faltantes (1 por email) → Preparar datos del nuevo usuario → Crear usuario en Snipe-IT (POST /users) → Registrar usuario creado → ¿Tiene ID? → Guardar usuario creado en mapa (BD) ┐ ↘ Usuarios listos (colapsa M→1, 46 nodos) →┐
     ...
     → ¿Titular ya mapeado en BD? (IF existing_user_id) → Usar usuario del mapa (Set) ──┐
                                         ↓ false → Marcar usuario no disponible (Set, status error) ──┐
                                                                                                     ↓ (fan-in 2→1)
                                                                                                  Preparar asignación (Set, solo rama ok) → Asignar activo al titular (checkout) (POST /checkout) → Registrar asignación exitosa (Set) ──┐
...
- **Precarga bulk de usuarios (Fase A, antes del diff):** `Precargar usuarios de Snipe-IT` (`GET /api/v1/users?limit=500`, `httpBearerAuth` `Adhjdtilu8D9eQs8`, `retryOnFail 3×2s`, 1 petición para ~74 usuarios) → `Guardar mapa de usuarios (bulk)` (`INSERT ... SELECT DISTINCT ON (LOWER(x.email)) ... FROM jsonb_to_recordset($1) AS x(email text, id int) WHERE x.email<>'' ORDER BY LOWER(x.email), x.id ON CONFLICT (email) DO UPDATE`, `$1 = body.rows.map(email=>LOWER(email), id)` filtrado no vacío) → `Calcular usuarios faltantes` (`DISTINCT ON LOWER(email)` no en mapa → `{payload: [...]|null}`) → `¿Hay usuarios faltantes?` (`payload` notEmpty) → `Extraer usuarios faltantes` (Code, 1 por email distinto con `username`/`password`) → `Preparar datos del nuevo usuario` (lee `$json`, fallback `email.split('@')[0]`) → `Crear usuario en Snipe-IT` (`POST /api/v1/users`, `onError→Crear alt`, batching 5/1000ms) → `Registrar usuario creado`/`(alt)` (`created_user` true, `snipe_user_id={{ $json.body?.payload?.id ?? $json.body?.id ?? $json.body?.data?.id ?? $json.payload?.id }}` fallback) → `¿Tiene ID?` (`Has User ID?` IF `snipe_user_id` isNotEmpty) → `Guardar usuario creado en mapa (BD)`; sin id → `Marcar ID faltante` (`Mark Missing ID`, `status error`, `Snipe no retornó snipe_user_id`); ambos fallan o sin id → `Marcar error al crear usuario`/`Marcar ID faltante` → `Usuarios listos` (Code colapsa M→1, fan-in 3→1: `Save New User`+`Mark Create Failed`+`Mark Missing ID`, 46 nodos, `Save New User` `[8032,2176]`). Rama `false` del IF va directo a `Contar activos sin mapeo`. **Eliminados 2026-09-03:** `Buscar usuario por email en Snipe-IT`, `¿Usuario existe en Snipe-IT?`, `Usar usuario existente de Snipe-IT`.
- `¿Titular ya mapeado en BD?` / `Usar usuario del mapa` / `Marcar usuario no disponible`: resuelven `snipe_user_id` **sin HTTP**. Si `existing_user_id` existe (cache tras precarga+creación) → `Usar usuario del mapa`; si no → `Marcar usuario no disponible` (`status error`, sin checkout). `Detectar cambios de titular` añade guarda `s.email<>''` y join `LOWER(s.email)`.
- `Preparar asignación` (Set, fan-in 4→1) → `Asignar activo al titular (checkout)` (`POST /api/v1/hardware/{id}/checkout` `{checkout_to_type:user, assigned_user, note}`, `retryOnFail 3×2s`, `batching 5/1000ms`, `onError: continueErrorOutput` → `Registrar asignación exitosa`) en error → `Liberar activo (checkin)` (`POST /checkin`, `onError: continueRegularOutput`) → `Reintentar asignación` (`batching 5/1000ms`) → `Registrar asignación exitosa (tras reintento)` / `Registrar error de asignación` (`status error`).
