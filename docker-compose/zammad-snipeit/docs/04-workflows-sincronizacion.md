...
| `n8n-remap.sh push` falla `no hay acceso a docker-postgres-1` en Mac lab | Fijaba el contenedor del compose Linux; el postgres de n8n del Mac lab es `dbs-postgres` (misma clase que `reset-sync.sh` 2026-09-08), y el push a casa del flujo Zammad exige `zammad`/`zammad_header` en los mapas locales. > **Fix 2026-09-15 (v1.3.1):** auto-detección (`docker-postgres-1`/`dbs-postgres`, precedencia `--db-cont` > `N8N_DB_CONT` > auto) + aviso si el mapa destino no define `zammad_header`; push verificado del flujo Zammad (`pg_n8n:2 bearer:2 header:1`, `wf_fuera:0`) + restart. Espejo de `.ai/specs/tryton-activos.md`. |
| Merge `cf62c77` reintroduce los 7 `envs/*.env` con secretos pese a `.gitignore` (push desde clon casa sin rebase) | Merge `cf62c77` (2026-09-14) une `45d7a71` (línea antigua con 7 `.env` trackeados) y `6f691a1` (línea saneada solo `.example`); el árbol del merge conserva los 7 `.env` y `origin/main` vuelve a rastrearlos; `.gitignore` solo ignora *untracked*. > **Fix 2026-09-16:** segunda purga con `git-filter-repo --invert-paths` de los 7 `envs/*.env` (`9800684…1dae2e4`), `force-push` de `main`, `fetch && reset --hard origin/main`, rotación de passwords DB consistentes y `ALTER USER` + `compose up -d`; `git ls-files --cached` solo 9 `.example`; flujo obligatorio `fetch --all && rebase` antes de `push`. Espejo de `tryton-activos.md`. |
...
- **Contrato UX (2026-09-14, 5 líneas `snipe_asset_summary`; 2026-09-15 tabla ASCII `note_body`):** `snipe_asset_summary` usa bloque `ACTIVOS SNIPE-IT (N)` + `--------------------` + N×(`NN modelo`/`   categoría`/`   tag`/`   estado`/`   S/N serial`) ordenados por `asset_tag` (5 líneas por activo, `categoría=r.category.name` identifica mouse/monitor/etc, blank line entre bloques, una línea por dato sin etiquetas largas, valores reales Snipe-IT sin conversión `OK`/`MAL`; vacío si `N=0`); `note_body` cuando `N>0` es tabla ASCII `ACTIVOS SNIPE-IT (N)` + `+---+`/`| # | MODELO | CATEGORIA | TAG | ESTADO | S/N |`/`+---+` + filas `| 01 | modelo | categoría | tag | estado | serial |` (anchos `max(header, celdas)` +2, celdas sanitizadas `\r/\n/\t/|→/`, `padEnd`, monospace `text/plain`, alineación perfecta).
...
- **Nodos Code — clean code (2026-09-14):** los 3 `Code` llevan manual en cabecera
  ...
  `FALLBACK_MAX_ID=9e15`), `Build Enrichment` (`isFetchError`/`sortByAssetTag`/`formatAssetBlock`/`buildAsciiTable`/`buildSummaryBlock`,
  `MSG_FETCH_ERROR`/`MSG_NO_ASSETS`/`FALLBACK_MODEL/CATEGORY/STATUS/TAG/SERIAL`, `TABLE_HEADERS=['#','MODELO','CATEGORIA','TAG','ESTADO','S/N']`, helpers arrow `sanitizeCell`/`padCell`,
  `summary` 5 líneas/bloque con `r.category.name`, `note` tabla ASCII con anchos `max(header, celdas)` +2), `Build Empty (no user)`
  (`MSG_SNIPE_FETCH_ERROR`/`buildMissingUserNote`). Verificado con harness
  (duplicados id menor, error red, 4 activos reales, fallbacks `Modelo?/Categoría?/Estado?/s/n`, tabla alineada). Espejo de `.ai/specs/zammad-tickets.md`.
  > **Fix 2026-09-14 (categoría):** `Build Enrichment` añade `r.category.name` como 2ª línea por activo.
  > **Fix 2026-09-15 (tabla ASCII nota):** `Build Enrichment` separa `summary` (bloque 5 líneas) de `note` (tabla ASCII `TABLE_HEADERS`/`sanitizeCell`/`padCell`/`buildAsciiTable`, anchos dinámicos, alineación perfecta, sin columnas torcidas).
