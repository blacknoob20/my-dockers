...
> tag `1410107-000650`, categoría `EQUIPO DE PROYECCION`) y el ticket descartable id 3 / `72003` (verificado 2026-09-14 con formato 5 líneas: `01 POWERLITE118`/`   EQUIPO DE PROYECCION`/`   1410107-000650`/`   good: Bueno`/`   S/N s/n`). Checklist
...
| `push --to casa` con `n8n-remap: no hay acceso a docker-postgres-1` en Mac lab | Script < v1.3.1 fijaba el contenedor Linux `docker-postgres-1`; en Mac lab el postgres de n8n es `dbs-postgres`. > **Fix 2026-09-15 (v1.3.1):** auto-detección del contenedor; el push del flujo a casa además requiere `zammad` en `envs/n8n-workflows.casa.json` y `zammad_header` en `envs/n8n-creds.casa.json` (sin la 2ª, el push deja el ID del snapshot y avisa). |
