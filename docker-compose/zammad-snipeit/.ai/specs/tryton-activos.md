...
| Tryton sync snipe-IT users assets (titular-activo v1) | Sub-workflow de negocio | `flows/flujos-dev/Tryton sync snipe-IT users assets.json` | `Yv8AlEkGEdwzRJrJ` |
...
| Zammad enrich ticket assets (bajo demanda) | Webhook (no sub-workflow Tryton) | `flows/zammad/Zammad enrich ticket assets.json` | `bgi4JmNIH5wNylf6` (trabajo, activo) |
...
### Zammad enrich ticket assets (bajo demanda)

Sin sync Snipe-IT → Zammad: al crear un ticket, webhook → n8n consulta el
estado **actual** en Snipe-IT (`users?email` → match LOWER exacto →
`hardware?assigned_to+assigned_type`) y hace `PUT` al ticket con los customs
`snipe_asset_count/tags/summary` + nota interna. Solo `ticket created`
...
Formato UX 2026-09-14 (5 líneas, `snipe_asset_summary`): `ACTIVOS SNIPE-IT (N)` + `--------------------` + N×(`NN modelo`/`   categoría`/`   tag`/`   estado`/`   S/N serial`), orden `asset_tag`, categoría=`r.category.name` (identifica mouse/monitor/etc), datos reales Snipe-IT sin conversión (ver spec Zammad). `note_body` desde 2026-09-15 usa tabla ASCII `ACTIVOS SNIPE-IT (N)` + `+---+`/`| # | MODELO | CATEGORIA | TAG | ESTADO | S/N |`/`+---+` con anchos `max(header, celdas)` +2 y celdas sanitizadas (`padEnd`, alineación perfecta).
...
> **Fix 2026-09-15 (tabla ASCII nota):** `Build Enrichment` separa `summary` (bloque 5 líneas) de `note` (tabla ASCII con `TABLE_HEADERS`/`sanitizeCell`/`padCell`/`buildAsciiTable`, anchos dinámicos, sin columnas torcidas).
...
> **Fix 2026-09-07 (fase 3 — workflowIds por ambiente):** el snapshot (sabor casa) traía `workflowIds` casa (`QHc2/aOao/5Oes/S87X/ym0z/Yv8A`) que en trabajo son huérfanos. `scripts/n8n-remap.sh` v1.2.0 ya remapea los 6 `Execute` por nombre exacto de nodo al mapa `envs/n8n-workflows.{casa,trabajo}.json` (nueva clave `orchestrator`: casa `BFfvossXQY8Ck5zh`, trabajo `3hh7DBsrq8A1rIQg`; actualiza `value`/`cachedResultUrl`/`cachedResultName`). En trabajo el vivo `PFlL1vCJhhGR6qAr` (mismo nombre, `workflowIds` mezclados casa/trabajo, 1 run) se archivó y `3hh7DBsrq8A1rIQg` (v2 batch, 6 IDs trabajo, 24 runs) se renombró al nombre canónico y se actualizó vía push al snapshot incremental (22 nodos). Nota: `users` es `Yv8AlEkGEdwzRJrJ` en casa y `yjyUYjVEaZ9UniSs` en trabajo (objetos distintos, ambos válidos en su ambiente).
...
- **Archivo:** `flows/flujos-dev/Tryton sync snipe-IT users assets.json` (snapshot sabor casa, ID `Yv8AlEkGEdwzRJrJ`; en trabajo el vivo es `yjyUYjVEaZ9UniSs`, activo)

> **Fix 2026-09-07 (rename snipe-IT):** flujo `Tryton sync users assets` → `Tryton sync snipe-IT users assets` (consistente con el resto) vía `git mv` + nodo orquestador `Execute Tryton sync snipe-IT users assets` (def + conexión + `cachedResultName`) + `users.name` en mapas + `NODOS_WF` en `n8n-remap.sh` v1.2.1; vivos renombrados con historial en trabajo (`yjyUYjVEaZ9UniSs`).
...
- **Invocado por:** orquestador v2 vía `Execute Tryton sync snipe-IT users assets` (`Execute Workflow` → ID del ambiente —casa `Yv8AlEkGEdwzRJrJ`, trabajo `yjyUYjVEaZ9UniSs`—, `waitForSubWorkflow: true`, input `{ok}`)
...
- **Decisión full/incremental automática (sin variable de entorno):** `Get last sync` (Postgres) devuelve `since` = `GREATEST(MAX(created_at) de integration_sync_log titular run_summary, MAX(finished_at) de tryton_snipe_run_summary) − 2h` + `map_rows` (`COUNT(snipe_titular_map)`); `Read Owned Assets` hace **full** solo si `map_rows = 0` o `since` es NULL (primer run, `reset-sync`, pérdida total del mapa), e **incremental** (`OR(write_date,create_date) >= since`) en el resto. Forzar full de users = vaciar/truncar `snipe_titular_map`; forzar full de assets = truncar `tryton_snipe_run_summary` (manual §9.3).
...
- `Preparar asignación` (Set, 1 inbound: rama ok de `Owner Mapped?` — absorbe el rename `existing_user_id`→`snipe_user_id` del eliminado `Use Mapped User`; `Marcar usuario no disponible` sale directo a `Package Results`) + `current_user_id`; checkpoint antes del IF `Not Needs Checkin?` (`current_user_id` vacío: true→`Checkout Asset` optimista con el fallback checkin+reintento intacto; false→`Checkin Asset` directo — salta el checkout condenado al fallo en reasignaciones); `Asignar activo al titular (checkout)` (`POST /api/v1/hardware/{id}/checkout` `{checkout_to_type:user, assigned_user, note}`, `retryOnFail 3×2s`, `batching 1/600ms` (~100/min, bajo el cap real 120/min de Snipe-IT), `onError: continueErrorOutput`) → `Registrar asignación exitosa` (`status ok`); en error → `Liberar activo (checkin)` (`POST /checkin` `{note}`, `onError: continueRegularOutput`) → `Reintentar asignación` (mismo body, `retryOnFail 3×2s`, `batching 1/600ms`) → `Registrar asignación exitosa (tras reintento)` / `Registrar error de asignación` (`status error` con `error_message/response_status/response_body` del `error`/`body`). `settings.executionTimeout: 7200` (el full de 8,991 checkouts tarda ~93 min; n8n solo evalúa el timeout entre nodos). Corre en proceso principal de n8n (sin límite `N8N_RUNNERS_TASK_TIMEOUT` del task runner).
...
- **Invocado por:** orquestador v2 vía `Execute Tryton sync snipe-IT assets` (`waitForSubWorkflow: true`, ID del ambiente —casa `ym0zaIpEj3J8xg2l`, trabajo `v6K5kipkr7UKp0fE`—)
...
Resumen por ejecución del orquestador. Esquema en `public.tryton_snipe_run_summary` (BD `n8n`). **Fix 2026-08-28:** no existía; `Run summary` fallaba. Añadida a `sql/init-sync-tables.sql` §7 y aplicada.

> **Fix 2026-09-07:** renombrada `sync_run_summary` → `tryton_snipe_run_summary` por estándar `tryton_*` (`Get last sync` es watermark incremental `MAX(finished_at)`, no el mapa de assets que es `tryton_snipe_asset_map`). Rename puro con `ALTER TABLE` (6 filas preservadas, sin `VIEW`); actualizados `sql/init-sync-tables.sql` §7, `Get last sync` (orquestador + users), `Run summary` (assets) y `scripts/reset-sync.sh`.
...
| Cap 120/min de Snipe-IT provoca 429s (~0.7% bajas con retry agotado) aunque n8n corra a 50/min (ráfagas de retry/self-heal sobre ventana deslizante) | Snipe-IT limita por usuario/token (`RouteServiceProvider`: `Limit::perMinute(config('app.api_throttle_per_minute'))`, default 120, header `X-RateLimit-Limit` verificado en vivo). > **Fix 2026-09-06:** aplicado en caliente sin downtime vía `docker exec` + `php artisan config:cache` persistente por env (verificado header `X-RateLimit-Limit`, run 506 en curso intacto). Pendiente tras FIN de 506: acelerar n8n a ~200/min (`batch 1/300ms`) → full ≈2h vs 7.5h, con ráfaga de prueba autocontenida. Nota prod: dimensionar PHP/MariaDB si se sube el cap. > **Medición 2026-09-10:** el vivo corre con el default 120 (`bootstrap/cache/config.php` = 120; la variable de subida se perdió del env y del contenedor) — la mejora pendiente quedó sin efecto; los batchings del sync se fijan ≤100/min por debajo del cap real. |
...
| `sync_run_summary` fuera del estándar `tryton_*` (`Get last sync` es watermark, no mapa) | `sync_run_summary` no empezaba por `tryton_` como `tryton_snipe_asset_map`/`staging_tryton_assets`; `Get last sync` (orquestador + users) lee `MAX(finished_at)` de esa tabla, no del mapa. > **Fix 2026-09-07:** rename puro `sync_run_summary` → `tryton_snipe_run_summary` (`ALTER TABLE`, 6 filas preservadas, sin `VIEW`); actualizados `sql/init-sync-tables.sql` §7, `Get last sync` (2 flujos), `Run summary` (assets) y `scripts/reset-sync.sh`; spec + `docs/04` + manual + `AGENTS.md` sincronizados. |
| `Workflow does not exist` en sub-workflow tras push a trabajo | `Tryton Login` (users) y `Execute login` (status) llaman a `Tryton login` con el `workflowId` del snapshot (casa `QHc2NJoBedOUJaPp`); `NODOS_WF` de `n8n-remap.sh` v1.2.1 solo cubría los 6 `Execute` del orquestador → el push llevó el ID casa al vivo trabajo `yjyUYjVEaZ9UniSs` (execs 1515/1519/1518/1522). > **Fix 2026-09-07:** alias `Tryton Login → login` en v1.2.2 (vale para `Execute login` de status); re-push users a `yjyUYjVEaZ9UniSs` + auditoría 0 colgadas + prueba orquestador incremental exec 1523 `success` 21s (`total=0/api_errors=0`). |
| `reset-sync.sh` aborta: 9948 assets usan esos modelos | El “paso 0” (`DELETE FROM assets`) solo existía en docs (AGENTS + spec Fix 2026-09-06); en código nunca estuvo (verificado en historial) y el guard de modelos abortaba antes de mutar nada. > **Fix 2026-09-07:** paso 0 real en el script (cuenta + `DELETE FROM assets` + `AUTO_INCREMENT=1` + verificación, LAB ONLY sin respaldo por decisión del usuario); reset verificado: Snipe 0 assets/0 modelos/cats 2/status 3 built-ins, 10 tablas PG en 0. |
...
| IDs del snapshot (casa) no coinciden con el vivo de trabajo — parece que `n8n-remap.sh` no remapeó | Snapshots commiteados en sabor casa (orquestador `BFfvossXQY8Ck5zh`, users `Yv8AlEkGEdwzRJrJ`); en trabajo los vivos son otros objetos (orquestador `3hh7DBsrq8A1rIQg`, users `yjyUYjVEaZ9UniSs`). Comparar snapshot contra vivo a mano sugiere un remap incompleto. > **Fix 2026-09-08:** verificado en vivo (`workflow_entity`) que los 6 `workflowId` del orquestador trabajo coinciden exacto con `envs/n8n-workflows.trabajo.json` (`wf_fuera:0`); el script sí traduce. Aclaración por ambiente añadida en § titular y `docs/04` §4.6. Verdad = `workflow_entity` en vivo, no el snapshot. |
| `n8n-remap.sh push` falla `no hay acceso a docker-postgres-1 (n8n_user/n8n)` en Mac lab | El push fijaba el contenedor `docker-postgres-1` (compose Linux) y el postgres de n8n del Mac lab es `dbs-postgres` (misma clase que el bug de `reset-sync.sh` 2026-09-08); además el push a casa del flujo Zammad exige `zammad` en `envs/n8n-workflows.casa.json` y `zammad_header` en `envs/n8n-creds.casa.json` (mapas locales aún v1.2.x). > **Fix 2026-09-15 (v1.3.1):** auto-detección del contenedor (`docker-postgres-1` Linux / `dbs-postgres` Mac; precedencia `--db-cont` > `N8N_DB_CONT` de `n8n-db.env` > auto) + aviso en push si el mapa destino no define `zammad_header`; mapas casa completados (`zammad`=`vzGTLv1QLUOaacQy`, `zammad_header`=`OVGJCA4EKgJUTgrX`). Push verificado: `vzGTLv1QLUOaacQy` `4d663c3e…`→`C15702D5…` (pg_n8n:2 bearer:2 header:1, `wf_fuera:0`) + restart y `/healthz` OK. |
| Merge `cf62c77` reintroduce los 7 `envs/*.env` con secretos pese a `.gitignore` (push desde clon casa sin rebase) | Merge `cf62c77` (2026-09-14) une primer padre `45d7a71` (línea antigua con 7 `.env` trackeados) y segundo padre `6f691a1` (línea saneada `6db1cc7` solo `.example`); el árbol del merge conserva los 7 `.env` y `origin/main` (`9800684`) vuelve a rastrearlos; `.gitignore` solo ignora *untracked*, no desrastrea archivos ya en el índice. Síntoma: `git ls-files` lista 7 `.env` y `git status --ignored` 0; `n8n.env` local con 5 líneas nuevas queda bajo seguimiento. > **Fix 2026-09-16:** segunda purga con `git-filter-repo --invert-paths` de los 7 `envs/*.env` (historia reescrita `9800684…1dae2e4`), `force-push` de `main`, `git fetch && git reset --hard origin/main` en clones, restauración de `envs` reales desde backup y rotación de passwords DB consistentes (`N8N_PG_PASS` para `n8n-db.env`↔`n8n.env`, `SNIPE` para `snipe-db.env`↔`snipe-it.env`, `ZAMMAD` para `zammad-db.env`↔`zammad-app.env`) + `ALTER USER` en `docker-postgres-1`/`docker-mariadb-1` (`mariadb_root_2024` para conectar) + `compose up -d` y verificación `git ls-files --cached` solo 9 `.example` y `curl 200` en snipe/zammad/n8n; tokens `SNIPEIT/ZAMMAD` requieren regeneración manual vía UI si estuvieron expuestos; flujo de trabajo obligatorio: `fetch --all && rebase` antes de `push`, nunca subir un clon casa sin rebase; backup en `/tmp/opencode-backup-envs-20260916-091829`. Espejo de `docs/04` §4.8. |
| Run 1830 de users: timeout evaluado entre nodos mata los upserts finales aunque `Checkout Asset` termina (mapa en 0, deadlock de full) | Ejecución 1830 (`yjyUYjVEaZ9UniSs`, 2026-09-10 14:11→18:00 UTC, disparada por el orquestador 1784): `Checkout Asset` procesó las 8,991 llamadas (`batching 1/1500ms`, 3h47m54s) y al volver al loop n8n vio `executionTimeout:3600` vencido — el timeout solo se evalúa entre nodos, así que el run largo sobrevivió al nodo y fue cancelado (`TimeoutExecutionCancelledError`) un nodo antes de `Checkout OK`→`Package Results`→`Upsert Owners (bulk)`. Tablas: `staging_titular`=8991, `snipe_titular_user_map`=699, **`snipe_titular_map`=0**, 0 filas titular en `integration_sync_log`. Inspección de `execution_data` 1830 (17.6MB): las 8,991 respuestas de checkout fueron HTTP 200 `{status:"error"}` "That asset is not available for checkout!" (6,122 ya asignados al titular + 2,869 con status no-deployable Malo/Baja/Desuso/Sin especificar, verificados en MariaDB: los 6,122 con email == titular, los 2,869 todos no-deployable). Causa raíz del bucle: con el mapa en 0, `Detect Changes` devolvía los 8,991 cada corrida y todo barrido completo manual excedía el timeout en ese mismo punto (runs 1592/1638/1830); además el cap real de Snipe-IT es 120/min (default; la subida a 600 de 2026-09-06 se perdió), así que un full de 8,991 NUNCA cabía en 3600s (mínimo físico 75 min). > **Fix 2026-09-10:** backfill SQL `snipe_titular_map` desde staging+asset_map+user_map (dry-run 8991/8991, apply 8,991 filas por `ON CONFLICT`: 6,122 certificados contra Snipe real + 2,869 "titular deseado" no-deployables — no se reintentarán aunque cambien de estado, documentado); decisión full/incremental automática sin variable de entorno (retirada por decisión de diseño: `Get last sync` de users con watermark propio `since` = `GREATEST(titular run_summary, assets summary)−2h` + `map_rows`; full si mapa vacío o sin watermark, forzar full = truncar `snipe_titular_map`/`tryton_snipe_run_summary`, manual §9.3; assets: `is_full = !since`); `batching 1/600ms` (~100/min, bajo el cap real 120/min) en `Checkout/Retry/Checkin Asset` + `settings.executionTimeout: 7200` (el full cabe en ~93 min); variables y docs limpiados (`envs/n8n.env(.example)`, `AGENTS.md`, spec, `docs/04`, manual §9.3); snapshots + vivos trabajo actualizados + recrear n8n. Espejo en `docs/04-workflows-sincronizacion.md` §4.8. |
