...
¿Titular ya mapeado en BD? (IF existing_user_id notEmpty) → Usar usuario del mapa (Set, snipe_user_id=existing, created_user=false) →┐
                                                    Buscar usuario por email en Snipe-IT (GET /api/v1/users?email=&limit=1, retry 3×2s, onError→Preparar) → ¿Usuario existe en Snipe-IT? (IF body.rows[0].id notEmpty) → Usar usuario existente de Snipe-IT (Set, created_user=false) →┤
                                                                                                                                   ↓ false/error → Preparar datos del nuevo usuario (Set, username/password) → Crear usuario en Snipe-IT (POST /api/v1/users, onError→Crear alt) → Registrar usuario creado (Set, created_user=true) →┤
                                                                                                                                                                                                                        ↓ error → Reintentar con username alternativo (POST /users, username con sufijo) → Registrar usuario creado (alt) (Set) →┤
                                                                                                                                                                                                                                                                  ↓ error → Marcar error al crear usuario (Set, status error) →┐
                                                                                                                                                                                                                                                                                                              ↓ (fan-in 4→1)
                                                                                                                                                                                                                                             Preparar asignación (Set) → Asignar activo al titular (checkout) (POST /api/v1/hardware/{id}/checkout, retry 3×2s, onError→Liberar activo (checkin)) → Registrar asignación exitosa (Set, status ok) →┐
                                                                                                                                                                                                                                                                                                                         ↓ error → Liberar activo (checkin) (POST /checkin) → Reintentar asignación (POST /checkout, retry 3×2s) → Registrar asignación exitosa (tras reintento) (Set, status ok) →┤
                                                                                                                                                                                                                                                                                                                                                                   ↓ error → Registrar error de asignación (Set, status error) →┤
                                                                                                                                                                                                                                                                                                                                                                                                        ↓ (fan-in 4→1)
                                                                                                                                                                                                                                                                                      Consolidar resultados (NoOp, fan-in: ok + ok reintento + error checkout + error usuario → {snipe_asset_tag, email, snipe_user_id, snipe_asset_id, status:'ok'|'error', created_user, error_message, response_status})
        ↓ (fan-out 1→2)
  ┌─────┴─────┐
  ↓           ↓
¿Asignación exitosa? (IF $json.status=='ok')   Calcular resumen de sincronización (Code: cuenta ok/error/omitidos desde $('Detectar cambios de titular')/$('Contar activos sin mapeo')/$('Consolidar resultados') → {cambios, aplicados, errores, omitidos, usuarios_nuevos})
 ↓ true/false                        ↓
 Guardar usuario en mapa (BD) (INSERT INTO snipe_titular_user_map ON CONFLICT(email) DO UPDATE)  Guardar resumen en bitácora (INSERT INTO integration_sync_log operation='run_summary', entity='titular')
 Guardar titular en mapa (BD) (INSERT INTO snipe_titular_map ON CONFLICT(snipe_asset_tag) DO UPDATE)
 ↓ false
 Registrar error en bitácora (INSERT INTO integration_sync_log operation='titular_checkout', entity='titular', por cada error con response_status/response_body/error_message)
```
...
- `¿Titular ya mapeado en BD?` / `Buscar usuario por email en Snipe-IT` / `¿Usuario existe en Snipe-IT?` / `Usar usuario existente de Snipe-IT` / `Preparar datos del nuevo usuario` / `Crear usuario en Snipe-IT` / `Reintentar con username alternativo` / `Registrar usuario creado*` / `Marcar error al crear usuario`: resuelven `snipe_user_id` por fila. Si `existing_user_id` existe → `Usar usuario del mapa` (cache `snipe_titular_user_map`); si no, `Buscar usuario por email en Snipe-IT` (`GET /api/v1/users?email=&limit=1`, `httpBearerAuth` `Adhjdtilu8D9eQs8`, `retryOnFail 3×2s`, `onError: continueErrorOutput` → `Preparar` en error de red) → `¿Usuario existe en Snipe-IT?` (`body.rows[0].id` notEmpty) → `Usar usuario existente de Snipe-IT`; si no encontrado → `Preparar datos del nuevo usuario` (genera `username` desde `email` + `password` aleatoria `Aa9!+rand`) → `Crear usuario en Snipe-IT` (`POST /api/v1/users`, `first_name/last_name` con fallback, `onError→Crear alt`) → `Registrar usuario creado`; en colisión de `username` → `Reintentar con username alternativo` (username con sufijo `.`+rand4) → `Registrar usuario creado (alt)`; si ambos fallan → `Marcar error al crear usuario` (`status error`). Todos los `Set` proyectan `{snipe_asset_tag, email, snipe_asset_id, snipe_user_id, created_user}`.
- `Preparar asignación` (Set, fan-in 4→1) checkpoint antes del checkout; `Asignar activo al titular (checkout)` (`POST /api/v1/hardware/{id}/checkout` `{checkout_to_type:user, assigned_user, note}`, `retryOnFail 3×2s`, `batching 20/500ms`, `onError: continueErrorOutput`) → `Registrar asignación exitosa` (`status ok`); en error → `Liberar activo (checkin)` (`POST /checkin` `{note}`, `onError: continueRegularOutput`) → `Reintentar asignación` (mismo body, `retryOnFail 3×2s`) → `Registrar asignación exitosa (tras reintento)` / `Registrar error de asignación` (`status error` con `error_message/response_status/response_body` del `error`/`body`). Corre en proceso principal de n8n (sin límite `N8N_RUNNERS_TASK_TIMEOUT` del task runner).
- `Consolidar resultados` (NoOp, fan-in 4→1 de `Registrar asignación exitosa` + `Registrar asignación exitosa (tras reintento)` + `Registrar error de asignación` + `Marcar error al crear usuario`) preserva contrato `{snipe_asset_tag, email, snipe_user_id, snipe_asset_id, status:'ok'|'error', created_user, error_message, response_status, response_body}` para `¿Asignación exitosa?`/`Calcular resumen de sincronización`.
- `¿Asignación exitosa?` filtra `status=='ok'` → upserts; `error` → `Registrar error en bitácora` en `integration_sync_log`.
- `Calcular resumen de sincronización` → `Guardar resumen en bitácora` deja siempre una fila `run_summary` (`entity='titular'`) con `{cambios, aplicados, errores, omitidos, usuarios_nuevos}` para observabilidad (visibilidad que antes solo vivía en `staticData.lastRun` y se perdía en runs manuales o fallos tempranos).
